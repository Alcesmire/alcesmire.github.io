...
function updateloop(){
  ...
  var url = "http://mlpol.000webhostapp.com/list.php";
  ...
  xmlhttp.send();
  // Ask if a forced refresh is needed
  // will be activated on the server for a few minutes when a template got changed
  // (not when a new one will get added)
  var xmlhttp_fr = new XMLHttpRequest();
  var url_fr = "http://mlpol.000webhostapp.com/force_refresh.php";
  xmlhttp_fr.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
  ...
  xmlhttp_fr.open("GET", url_fr, true);
  xmlhttp_fr.send();
...
function loadImage(imagename){
  ...
  if(cachebreaker != null)
    image_list[imagename].src = "http://mlpol.000webhostapp.com/image.php?name=" + imagename + "&cachebreaker=" + cachebreaker;
  else
    image_list[imagename].src = "http://mlpol.000webhostapp.com/image.php?name=" + imagename;
